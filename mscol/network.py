...
class ContrastiveModel(tf.keras.Model):
  ...
  def call(self, data): #used for the validation subset
    data = tf.convert_to_tensor(data)
    anchors,positives = tf.split(data,2,axis=-1)
    
    zeros = tf.where(tf.reduce_sum(positives,axis=tf.range(1,3))==0)
    active = tf.where(tf.reduce_sum(positives,axis=tf.range(1,3))!=0)
...
def get_contrastive_network(embedding_dim,
  ...
  encoder = get_efficient_net_encoder(x.shape[1:], pooling_type)
  x = encoder(x)
